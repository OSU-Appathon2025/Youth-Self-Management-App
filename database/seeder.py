...
def seed_randomized():
    ...
    for _ in range(NUM_USERS):
        user = {
            ...
            "email": fake.email(),
            "full_name": fake.name(),
            "date_of_birth": str(fake.date_of_birth(minimum_age=14, maximum_age=18)),
        }
        users.append(user)
    ...
    for user in users:
        ...
        for g in random.sample(goal_templates, 2):
            goals.append({
                ...
                "category": g[1],
                "target_date": date.today() + timedelta(days=random.randint(7, 30)),
        ...
        for name in random.sample(["Albuterol", "Insulin", "Adderall", "Zyrtec"], random.randint(1, 2)):
            meds.append({
                ...
                "next_refill_date": date.today() + timedelta(days=random.randint(5, 20)),
